...
const supplierQ = require('./supplier-questionnaires');
...
// === Job: recurring questionnaire re-attestation ===
// For each active schedule whose next_due_date has arrived, issue a fresh questionnaire
// (new row + new token) to the target supplier(s), then advance next_due_date by the
// cadence. The issue + advance run in one transaction so an overlapping hourly tick
// cannot double-send. Title is day-count-free so notify()'s dedup holds.
function jobRecurringQuestionnaires() {
  const due = db.prepare(`SELECT * FROM recurring_questionnaire_schedules
    WHERE active=1 AND next_due_date IS NOT NULL AND next_due_date <= date('now')`).all();
  let count = 0;
  for (const sch of due) {
    const ws = db.prepare(`SELECT id, brand_display_name, client_name, firm_id FROM workspaces WHERE id=?`).get(sch.workspace_id);
    if (!ws) continue;
    const brandName = ws.brand_display_name || ws.client_name;
    let targets;
    if (sch.supplier_id) {
      targets = db.prepare(`SELECT id FROM suppliers WHERE id=? AND workspace_id=? AND lifecycle_stage != 'terminated'`).all(sch.supplier_id, sch.workspace_id);
    } else {
      let q = `SELECT id FROM suppliers WHERE workspace_id=? AND lifecycle_stage != 'terminated'`;
      const p = [sch.workspace_id];
      if (sch.tier_filter && sch.tier_filter !== 'all') { q += ` AND tier=?`; p.push(sch.tier_filter); }
      targets = db.prepare(q).all(...p);
    }
    const months = sch.cadence_months || 12;
    db.transaction(() => {
      targets.forEach(s => supplierQ.issueQuestionnaire({
        workspaceId: sch.workspace_id, supplierId: s.id, templateId: sch.template_id,
        contactRole: sch.contact_role, brandName, firmId: ws.firm_id
      }));
      db.prepare(`UPDATE recurring_questionnaire_schedules SET last_sent_at=CURRENT_TIMESTAMP, next_due_date=date('now', '+' || ? || ' months') WHERE id=?`).run(months, sch.id);
    })();
    count += targets.length;
    if (targets.length) {
      notify(sch.workspace_id, null, 'recurring_questionnaire_sent', 'low',
        'Recurring questionnaires issued',
        `${targets.length} re-attestation questionnaire${targets.length === 1 ? '' : 's'} issued from a recurring schedule.`,
        `/workspaces/${sch.workspace_id}/vendors/dashboard`, 14);
    }
  }
  return count;
}
...
const JOBS = [
  ...
  ['recurringQuestionnaires',  jobRecurringQuestionnaires]
];
...
module.exports = { runAllJobs, start, stop, notify };
