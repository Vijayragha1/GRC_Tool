...
function runDrill() {
  ...
  try {
    // File layout (see lib/backup.js): 12-byte IV source + 16-byte GCM tag + ciphertext.
    const blob = fs.readFileSync(file);
    const ivSrc = blob.subarray(0, 12);
    const tag = blob.subarray(12, 28);
    const ct = blob.subarray(28);
    const decipher = crypto.createDecipheriv('aes-256-gcm', enc.masterKey(), Buffer.alloc(12, ivSrc.toString()));
