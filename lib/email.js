...
async function sendSupplierQuestionnaireEmail({
  ...
  const baseUrl = appBaseUrl();
  const startUrl = `${baseUrl}/q/${token}`;
  const greeting = toName ? `Hi ${toName},` : 'Hi,';
  ...
  const bodyHtml = `
    <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#27272a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">${escapeHtml(greeting)}</p>
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#51525c;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">As part of an ongoing security review, <strong>${escapeHtml(workspaceName)}</strong> has asked <strong>${escapeHtml(supplierName)}</strong> to complete a short security questionnaire. ${escapeHtml(countLine)}</p>
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 16px;border:1px solid #ecebe5;border-radius:6px;background:#fafaf8;">
      <tr><td style="padding:14px 18px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,system-ui,Arial,sans-serif;">
    ...
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#51525c;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">Click the button below to open it. You don't need an account &mdash; the link itself is your secure access. Your answers save as you go, so you can submit when you're ready.</p>`;
  ...
  const footnote = expiryStr
    ? `This link is unique to ${escapeHtml(supplierName)} and expires on <strong>${escapeHtml(expiryStr)}</strong>. Please don't forward it &mdash; anyone who has it can submit on your behalf.`
    : `This link is unique to ${escapeHtml(supplierName)}. Please don't forward it &mdash; anyone who has it can submit on your behalf.`;
