...
function seedTemplate(db, ws, userId, planId) {
  ...
  const insertDeliverable = db.prepare(`INSERT INTO engagement_delivery_deliverables
    (workspace_id,plan_id,milestone_id,title,description,acceptance_criteria,status,is_required,due_date,accepted_by,accepted_at,decision_note)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`);
  ...
  PHASES.forEach((phase, phaseIndex) => {
    ...
    phase.milestones.forEach((milestoneKey, milestoneIndex) => {
      ...
      insertDeliverable.run(ws.id, planId, milestoneId, source.deliverables || source.title,
        `Acceptance evidence for “${source.title}”.`, criteria, complete ? 'accepted' : 'draft', 1,
...
function sourceVerification(db, ws, readinessInput = null) {
  ...
  mark('w1-stakeholders', parties >= 3, `${parties} interested parties have documented needs`, `/workspaces/${wsId}/interested-parties`);
  ...
  mark('w2-crown', crown >= 3 && crown <= 5, `${crown} crown-jewel assets are classified critical`, `/workspaces/${wsId}/assets`);
...
function reviseDeliverable(db, ws, userId, deliverableId, note) {
  ...
  const tx = db.transaction(() => {
    ...
    const nextId = db.prepare(`INSERT INTO engagement_delivery_deliverables
      (workspace_id,plan_id,milestone_id,title,description,acceptance_criteria,status,is_required,owner_id,approver_id,due_date,
       linked_record_type,linked_record_id,client_visible,requires_evidence,revision_number,supersedes_deliverable_id)
      VALUES (?,?,?,?,?,?,'draft',?,?,?,?,?,?,?,?,?,?)`).run(row.workspace_id,row.plan_id,row.milestone_id,row.title,row.description,
        row.acceptance_criteria,row.is_required,row.owner_id,row.approver_id,row.due_date,row.linked_record_type,row.linked_record_id,
