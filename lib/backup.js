...
async function runBackup() {
  ...
  try {
    ...
    // IV MUST be random per backup: GCM catastrophically fails if a (key, IV)
    // pair is ever reused (it leaks the XOR of plaintexts and allows tag
    // forgery). The previous code derived the IV from the timestamp, which
    // collided for any two backups in the same window - never do that.
    // Envelope: [12-byte random IV][16-byte GCM tag][ciphertext]. The IV is
    // stored in the clear (standard for GCM) and read back by scripts/restore.js.
    ...
    fs.writeFileSync(finalPath, Buffer.concat([iv, tag, ct]));
...
// Verify the backup directory is actually writable at boot - a backup that
// silently fails to a read-only or missing path is worse than no backup
// because it gives false confidence. Fails loud, does not crash the server.
function assertWritable() {
  try {
    ensureDir();
    const probe = path.join(BACKUP_DIR, '.write-probe');
    fs.writeFileSync(probe, 'ok');
    fs.unlinkSync(probe);
    return true;
  } catch (e) {
    console.error(`[backup] FATAL: backup dir is not writable (${BACKUP_DIR}): ${e.message}`);
    console.error('[backup] No backups will be produced until this is fixed. Set ISMS_BACKUP_DIR to a writable, off-box-replicated path.');
    return false;
  }
}
