...
function transitionFinding(db,ws,actorId,id,action,body={}) {
  ...
  db.transaction(()=>{
    const result=db.prepare(`UPDATE consulting_findings SET status=?,owner_id=COALESCE(?,owner_id),due_date=COALESCE(?,due_date),
      remediation_plan=COALESCE(?,remediation_plan),resolution_summary=COALESCE(?,resolution_summary),validation_conclusion=COALESCE(?,validation_conclusion),
      ...
      updated_at=datetime('now'),row_version=row_version+1 WHERE id=? AND workspace_id=? AND row_version=?`).run(to,body.owner_id||null,body.due_date||null,
        clean(body.remediation_plan,15000),clean(body.resolution_summary,15000),clean(body.validation_conclusion,15000),to,actorId,to,to,actorId,to,row.id,ws.id,version);
...
function generateReport(db,ws,actorId,engagementId,body) {
  ...
  const findings=db.prepare(`SELECT finding_ref,title,finding_type,severity,condition_text,criteria_text,effect_text,recommendation_text,status,due_date,resolution_summary,validation_conclusion
