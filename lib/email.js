  ...
  const bodyHtml = `
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 16px;border:1px solid #ececef;border-radius:6px;">
      <tr><td style="padding:14px 18px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">
        ...
        <div style="font-size:15px;font-weight:600;color:#0a0a0a;margin-bottom:6px;">${escapeHtml(docName)} <span style="color:#9c9ca5;font-weight:400;">· v${docVersion}</span></div>
        ${roleLabel ? `<div style="font-size:12px;color:#71717a;margin-bottom:8px;">Acting as: ${escapeHtml(roleLabel)}</div>` : ''}
        ${changeSummary ? `<div style="font-size:13px;line-height:1.5;color:#51525c;border-left:2px solid #5C0A0A;padding-left:10px;margin-top:8px;">${escapeHtml(changeSummary)}</div>` : ''}
      </td></tr>
    </table>
...
async function sendPasswordResetEmail({ toEmail, toName, token, expiresAt, firmId = null }) {
  ...
  const bodyHtml = `
    <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#27272a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">${escapeHtml(greeting)}</p>
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#51525c;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">We received a request to reset the password on your ISMS account (<strong>${escapeHtml(toEmail)}</strong>). Use the button below to choose a new one.</p>
    <p style="margin:0 0 16px;font-size:13px;line-height:1.55;color:#71717a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Arial,sans-serif;">If you didn't ask for this, you can safely ignore the message &mdash; your existing password stays valid.</p>`;
  ...
  return sendEmail({
    to: toEmail,
    subject: '[ISMS] Reset your password',
    html: renderEmailLayout({
...
async function sendInviteEmail({ toEmail, toName, inviterName, firmName, role, token, expiresAt, firmId = null }) {
  ...
  return sendEmail({
    to: toEmail,
    subject: `[ISMS] You're invited to join ${firmName || 'the ISMS'}`,
    html: renderEmailLayout({
