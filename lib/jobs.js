...
// === Job: ack campaign reminders + close completed campaigns ===
function jobAckCampaignReminders() {
  const open = db.prepare(`SELECT * FROM doc_ack_campaigns WHERE status='active'`).all();
  let count = 0;
  for (const c of open) {
    const stats = db.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN acknowledged_at IS NOT NULL THEN 1 ELSE 0 END) AS done FROM doc_ack_recipients WHERE campaign_id=?`).get(c.id);
    if (stats.total > 0 && stats.done === stats.total) {
      db.prepare(`UPDATE doc_ack_campaigns SET status='completed', closed_at=CURRENT_TIMESTAMP WHERE id=?`).run(c.id);
      notify(c.workspace_id, null, 'ack_campaign_done', 'info', `Campaign complete: ${c.name}`, `${stats.done}/${stats.total} acknowledged.`, `/workspaces/${c.workspace_id}/campaigns/${c.id}`);
      continue;
    }
    if (c.due_date && c.due_date < new Date().toISOString().slice(0,10)) {
      const open = stats.total - stats.done;
      if (open > 0) {
        if (notify(c.workspace_id, null, 'ack_campaign_overdue', 'high',
          `Acknowledgement campaign overdue: ${c.name}`,
          `${open}/${stats.total} recipients have not yet acknowledged.`,
          `/workspaces/${c.workspace_id}/campaigns/${c.id}`)) count++;
      }
    }
  }
  return count;
...
const JOBS = [
  ...
  ['ackCampaignReminders',     jobAckCampaignReminders],
