...
function publicRequestStatus(value) {
  return ({
    ...
    accepted: 'Complete', changes_requested: 'More information required', cancelled: 'Cancelled'
...
function buildClientGapAssessmentProjection(db, workspace, options = {}) {
  ...
  const requestScope = options.assigneeId
    ? ' AND cr.assignee_id=?'
    : options.releasedOnly
      ? ' AND cr.assignee_id IS NOT NULL'
      : '';
  ...
  const findings = engagement ? db.prepare(`SELECT f.id,f.finding_ref,f.title,f.finding_type,f.severity,
    ...
    WHERE f.workspace_id=? AND f.engagement_id=? AND f.client_visible=1 AND f.status NOT IN ('draft','withdrawn')
    GROUP BY f.id ORDER BY CASE f.severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END,f.id`).all(workspace.id, engagement.id).map(row => ({
      ...row,
  ...
  const reports = engagement ? db.prepare(`SELECT id,title,report_type,version_number,published_at
    FROM consulting_report_snapshots
    WHERE workspace_id=? AND engagement_id=? AND status='published' AND report_type IN ('assessment','readiness')
    ORDER BY published_at DESC,id DESC`).all(workspace.id, engagement.id) : [];

  const currentPhase = governance.currentPhase;
  const phaseDefinitions = PHASES.filter(phase => governance.phases.includes(phase.key));
  const currentDefinition = phaseDefinitions.find(phase => phase.key === currentPhase);
  ...
  const assessmentDelivered = currentPhase === 'complete';
  const contractClosed = governance.closure.complete;

  return {
    applicable: true,
    currentPhase,
    currentPhaseLabel: contractClosed
      ? 'Engagement complete'
      : assessmentDelivered
        ? 'Report issued - ready for governed closure'
        : currentDefinition?.label || 'Engagement planning',
    currentPhaseDescription: currentPhase === 'complete'
      ? contractClosed
        ? 'The gap-assessment engagement is closed; open recommendations remain with your organisation.'
        : 'The independently approved report has been released. A workspace manager must still record the governed contract closure.'
      : currentDefinition?.description || 'The engagement team is preparing the agreed assessment work.',
    gapOnly: governance.gapOnly,
    assessmentDelivered,
    engagementComplete: contractClosed,
    contractClosed,
    phases,
    scope: {
