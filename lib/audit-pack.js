...
// Serialize PDF generation. Each launch spins up a full Chromium (~150-200MB);
// two concurrent renders on the recommended 1GB box invite the OOM killer
// exactly while demoing the flagship deliverable. This single-flight queue caps
// concurrency at 1 - callers still just `await renderPDF(...)` as before.
let _pdfChain = Promise.resolve();
function renderPDF(html, opts) {
  const run = () => _renderPDFNow(html, opts);
  const result = _pdfChain.then(run, run);
  // Keep the queue alive but isolated from this render's outcome.
  _pdfChain = result.then(() => {}, () => {});
  return result;
}

async function _renderPDFNow(html, opts) {
  opts = opts || {};
  const puppeteer = getPuppeteer();
  const browser = await puppeteer.launch({
    headless: 'shell',
    // --disable-dev-shm-usage: Docker's default /dev/shm is 64MB; without this
    // Chromium exhausts it on a large SoA/evidence table and crashes.
