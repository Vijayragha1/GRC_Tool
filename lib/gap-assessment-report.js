...
function buildGapAssessmentReportData(db, workspace, pass, options = {}) {
  ...
  const clauseGaps = clauseRows.filter(row => gapStatuses.has(row.status));
  const annexGaps = annexRows.filter(row => gapStatuses.has(row.status));

  let findingCounter = 0;
  const toFinding = row => {
    findingCounter += 1;
    const severity = row.nc
    ...
    return {
      ...
      severity,
      currentState: (row.nc && (row.nc.description || row.nc.title)) || row.notes || 'No assessment narrative was recorded for this gap.',
  ...
  const reportStatus = currentState ? 'WORKING STATUS' : (pass.status === 'completed' ? 'FINAL' : 'CONTROLLED DRAFT');
...
function findingHtml(finding) {
  return `<article class="finding">
    <div class="finding-head">
      <div><strong>${esc(finding.findingRef)}</strong> ${badge(/MAJOR|CRITICAL/.test(finding.severity) ? 'Not Implemented' : 'Partially Implemented')} <b>${esc(finding.cleanTitle)}</b></div>
...
function renderGapAssessmentHtml(data) {
  ...
  const executiveHeadline = data.notAssessedCount
    ...
    : (data.frameworkGaps
      ...
      : 'No implementation gaps were identified in the concluded assessment population.');
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${esc(data.reportId)} Gap Assessment</title>
    ...
    <section class="sheet cover">
      ...
      <div class="eyebrow">Readiness assessment - ${esc(data.reportStatus)}</div>
      <h1>ISO/IEC<br>27001:2022<br><span>Gap Assessment</span></h1>
      <p class="scope-deck">${esc(coverScope)}</p>
      <table class="metrics"><tr>
        ${metric('Overall readiness', `${data.readiness}%`, `weighted across ${data.rows.length} requirements`)}
        ${metric('Not assessed', data.notAssessedCount, `${data.rows.length - data.notAssessedCount} requirements have a conclusion`)}
        ${metric('Recorded gaps', data.frameworkGaps, `${data.annexFindings.length} controls - ${data.clauseFindings.length} clauses`)}
        ${metric('Certification target', data.targetDate, data.currentState ? 'working status snapshot' : (data.pass.status === 'completed' ? 'assessment pass concluded' : 'controlled draft'))}
      </tr></table>
      <table class="facts">
      ...
      <div class="notice"><strong>THIS IS NOT A CERTIFICATION AUDIT</strong>
        This report is a readiness assessment against ISO/IEC 27001:2022. It does not provide certification, legal advice, or an auditor's certification opinion. Findings reflect the evidence and assessment decisions recorded up to ${esc(formatDate(data.cutoff))}.${data.currentState ? ` This is a working point-in-time status report; ${data.notAssessedCount} requirements are explicitly reported as Not Assessed.` : ''}
      </div>
    </section>
    ...
    <section class="sheet">
      ...
      <table class="two-col"><tr><td>
        <p>${esc(statusNarrative)} The weighted readiness position is <strong>${data.readiness}%</strong>. This score is a directional portfolio indicator; the individual requirement conclusions and retained evidence remain authoritative.</p>
        ...
        <div class="score-box"><div class="eyebrow">Readiness at report date</div><div class="score">${data.readiness}%</div><div class="bar"><i style="width:${data.readiness}%"></i></div><small>${data.counts['Implemented']} implemented - ${data.frameworkGaps} with a gap - ${data.counts['Not Applicable']} not applicable</small></div>
    ...
    <section class="sheet">
      ...
      <p>A conclusion of Implemented indicates that the assessor recorded the requirement as implemented. It does not, by itself, prove operating effectiveness. Evidence references show the records linked to each requirement at the reporting cutoff.</p>
