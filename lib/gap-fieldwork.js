...
function assessmentContext(db, workspace) {
  ...
  const gapOnly = isoLifecycle.isGapOnly(workspace.engagement_outcome);
  ...
  const engagement = db.prepare(`SELECT id,scope_statement,engagement_type,status,row_version,completed_at,completion_note
    FROM consulting_engagements WHERE workspace_id=? AND status!='cancelled'
    ORDER BY CASE engagement_type WHEN 'gap_assessment' THEN 0 ELSE 1 END,
      CASE status WHEN 'active' THEN 0 WHEN 'quality_review' THEN 1 WHEN 'complete' THEN 2 ELSE 3 END,id DESC LIMIT 1`).get(workspace.id) || null;
  const findings = engagement ? db.prepare(`SELECT id,status,client_visible FROM consulting_findings
    WHERE workspace_id=? AND engagement_id=? AND client_visible=1 AND status NOT IN ('draft','withdrawn')`).all(workspace.id, engagement.id) : [];
  const reports = engagement ? db.prepare(`SELECT id,status,generated_by,approved_by,approved_at,published_by,published_at
    FROM consulting_report_snapshots WHERE workspace_id=? AND engagement_id=?
      AND report_type IN ('assessment','readiness')`).all(workspace.id, engagement.id) : [];
  ...
  const independentlyApprovedReports = reports.filter(row => row.status === 'published' && row.approved_by && row.approved_at &&
    row.published_at && Number(row.approved_by) !== Number(row.generated_by));
  const publishedReports = independentlyApprovedReports.length;
  const openRemediation = findings.filter(row => row.status !== 'closed').length;
  const openWorkpapers = engagement ? db.prepare(`SELECT COUNT(*) c FROM consultant_workpapers
    WHERE engagement_id=? AND status NOT IN ('frozen','superseded')`).get(engagement.id).c : 0;
  const openEngagementRequests = engagement ? db.prepare(`SELECT COUNT(*) c FROM client_requests
    WHERE engagement_id=? AND status NOT IN ('accepted','cancelled')`).get(engagement.id).c : 0;
  const interviewScheduleConcluded = interviews.length > 0 && interviews.every(row => CLOSED_INTERVIEWS.has(row.status));
  const rfiRegisterConcluded = requests.length > 0 && requests.every(row => RECEIVED_REQUESTS.has(row.status));
  ...
  const gates = {
    ...
    fieldwork: {
      ready: !!pass && pass.status === 'completed' && Number(control.assessed || 0) === Number(control.total || 0) &&
        interviewScheduleConcluded && rfiRegisterConcluded &&
      ...
      checks: [
        ...
        { label: interviews.length ? 'Interview schedule concluded' : 'At least one interview must be scheduled and concluded', pass: interviewScheduleConcluded },
        { label: requests.length ? 'All RFIs received or closed' : 'At least one documented RFI must be issued and resolved', pass: rfiRegisterConcluded },
    ...
    post_report: {
      ready: publishedReports > 0 && findings.length > 0 && openRemediation === 0,
      checks: [
        { label: 'Client report published', pass: publishedReports > 0 },
        { label: findings.length ? 'All confirmed findings closed' : 'No-findings outcome requires an explicit Not required decision', pass: findings.length > 0 && openRemediation === 0 }
      ]
    }
  };
  ...
  const completed = {
    ...
    post_report: gapOnly ? publishedReports > 0 : finalDecision('post_report')
  };
  const phases = gapOnly ? PHASES.slice(0, -1) : [...PHASES];
  let currentPhase = phases.find(phase => !completed[phase]) || phases[phases.length - 1];
  ...
  const assessmentDeliveryComplete = gapOnly && completed.mobilisation && completed.fieldwork && completed.validation && completed.report;
  if (assessmentDeliveryComplete) currentPhase = 'complete';

  const closureBlockers = [];
  if (!engagement) closureBlockers.push('A governed gap-assessment engagement is required.');
  else if (engagement.engagement_type !== 'gap_assessment') closureBlockers.push('The consulting engagement must be classified as a gap assessment.');
  if (!pass || pass.status !== 'completed') closureBlockers.push('Complete the independently reviewed assessment pass.');
  if (!completed.mobilisation) closureBlockers.push('Record mobilisation sign-off.');
  if (!completed.fieldwork) closureBlockers.push('Record fieldwork sign-off.');
  if (!completed.validation) closureBlockers.push('Record validation sign-off or a governed Not required decision.');
  if (!publishedReports) closureBlockers.push('Publish an independently approved assessment report.');
  if (openWorkpapers) closureBlockers.push(`Freeze or supersede ${openWorkpapers} open workpaper${openWorkpapers === 1 ? '' : 's'}.`);
  if (openEngagementRequests) closureBlockers.push(`Accept or cancel ${openEngagementRequests} open client request${openEngagementRequests === 1 ? '' : 's'}.`);
  ...
  return {
    ...
    gates, completed, currentPhase, phases, gapOnly,
    closure: {
      applicable: gapOnly,
      ready: gapOnly && closureBlockers.length === 0 && engagement?.status !== 'complete',
      complete: gapOnly && engagement?.status === 'complete',
      blockers: closureBlockers,
      openFindings: openRemediation,
      independentlyApprovedReports: publishedReports
    },
    live: {
