...
function buildContext(wsId) {
  ...
  const totals = db.prepare(`SELECT
    ...
    COUNT(*) total
    FROM control_states WHERE workspace_id=?`).get(wsId);
  const totalItems = db.prepare(`SELECT COUNT(*) c FROM iso_items`).get().c;

  const soa = {
    included: db.prepare(`SELECT COUNT(*) c FROM control_states cs INNER JOIN iso_items i ON i.id=cs.iso_item_id WHERE cs.workspace_id=? AND cs.applicability='included' AND i.type='control'`).get(wsId).c,
    excluded: db.prepare(`SELECT COUNT(*) c FROM control_states cs INNER JOIN iso_items i ON i.id=cs.iso_item_id WHERE cs.workspace_id=? AND cs.applicability='excluded' AND i.type='control'`).get(wsId).c,
    undecided: db.prepare(`SELECT COUNT(*) c FROM iso_items i LEFT JOIN control_states cs ON cs.iso_item_id=i.id AND cs.workspace_id=? WHERE i.type='control' AND COALESCE(cs.applicability,'undecided')='undecided'`).get(wsId).c
  };
  ...
  db.prepare(`SELECT 'training_expiry' AS k, training_name AS title, expiry_date AS d FROM training_records WHERE workspace_id=? AND expiry_date IS NOT NULL AND expiry_date <= date('now','+90 days')`).all(wsId).forEach(r => calendar.push(r));
