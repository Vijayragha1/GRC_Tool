...
function reqId(db, framework, ref) {
  const r = db.prepare(`SELECT r.id FROM requirements r JOIN frameworks f ON f.id=r.framework_id
    WHERE f.code=? AND r.ref=? AND f.status='active' ORDER BY f.id DESC LIMIT 1`).get(framework, ref);
  return r ? r.id : null;
}
