...
const { FRAMEWORK_LIST } = require('./frameworks');
...
// Projected from lib/frameworks.js rather than re-declared. This block used to
// be a second copy of the programme labels, so a framework added to the
// registry appeared everywhere except the integrated dashboard, where it fell
// through to no programme card at all.
const FRAMEWORK_META = Object.fromEntries(FRAMEWORK_LIST.map(f => [f.code, {
  label: f.label,
  short: f.shortLabel,
  code: f.systemCode,
...
function isoProgramme(db, ws, frameworkCode) {
  ...
  const counts = db.prepare(`SELECT
    ...
    WHERE f.code=? AND f.status='active'`).get(ws.id, ws.id, frameworkCode, ws.id, frameworkCode);
  ...
  const isDpdpa = frameworkCode === 'dpdpa';
  ...
  let nextLabel = is27001 ? 'Open gap assessment' : (isDpdpa ? 'Open DPDPA gap assessment' : 'Open AI gap assessment');
  let nextHref = is27001 ? `${base}/gap-assessment` : (isDpdpa ? `${base}/dpdpa/current/assessment` : `${base}/iso42001/gap-assessment`);
  if (assessed === total && openItems > 0) {
    nextLabel = 'Open improvement roadmap';
    nextHref = is27001 ? `${base}/engagement-plan?view=timeline` : (isDpdpa ? `${base}/dpdpa/current/assessment` : `${base}/iso42001/roadmap`);
  } else if (assessed === total && openItems === 0) {
    nextLabel = 'Review readiness';
    nextHref = is27001 ? `${base}/readiness` : (isDpdpa ? `${base}/dpdpa/current/review` : `${base}/iso42001/readiness`);
  }
  return {
    ...
    programmeHref: is27001 ? `${base}/readiness` : (isDpdpa ? `${base}/dpdpa` : `${base}/iso42001/readiness`),
    nextLabel, nextHref,
  };
}
...
function csfProgramme(db, ws) {
  ...
  if (!engagement) {
    return {
      ...
      assessed: 0, total: 106, unassessedOutcomes: 106,
      openConfirmedFindings: 0, openItems: 0, evidence: 0,
  ...
  // Findings do not use the legacy Closed/Accepted lifecycle. Draft findings
  // are assessor working material, while Reviewed/Approved/Published findings
  // are confirmed governance records. Keep assessment coverage and confirmed
  // findings as separate facts so 106 unassessed outcomes can never be
  // presented to a client as 106 findings.
  const openConfirmedFindings = db.prepare(`SELECT COUNT(*) count FROM csf_findings
    WHERE engagement_id=? AND deleted_at IS NULL
      AND status IN ('Reviewed','Approved','Published')`).get(engagement.id).count;
  const unassessedOutcomes = Math.max(0, total - assessed);
  let status = 'Not started';
  ...
  return {
    ...
    unassessedOutcomes,
    openConfirmedFindings: Number(openConfirmedFindings || 0),
    openItems: Number(openConfirmedFindings || 0), evidence,
    documents: linkedArtifactCount(db, ws.id, 'csf', 'documents'),
...
function sharedSignals(db, wsId, today) {
  return db.prepare(`SELECT
    ...
    (SELECT COUNT(*) FROM generated_docs WHERE workspace_id=? AND retired_at IS NULL AND status!='withdrawn') documents,
...
function clientDecisionTruth(db, ws, today, actorId, clientFacing = false) {
  ...
  const phaseScope = outcomeScope.phaseSqlForWorkspace(ws, 'ph');
  const deliverables = db.prepare(`SELECT d.id,d.client_title,d.title,d.status,d.due_date,d.owner_id,d.approver_id,
    ...
    WHERE d.workspace_id=? AND ${phaseScope}
      AND d.client_visible=1 AND d.status NOT IN ('accepted','superseded')
