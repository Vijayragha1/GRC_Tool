...
function snapshotDocVersion(docId, wsId, status, userId, summary) {
  ...
  if (!doc) return null;
  // New immutable versions always hash the safe representation. Historical
  // versions are never rewritten; render-boundary sanitisation handles those.
  const decryptedContent = documentHtml.sanitizeDocumentHtml(enc.decryptIfNeeded(doc.content, wsId));
