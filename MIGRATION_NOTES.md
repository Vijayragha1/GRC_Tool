...
**Self-consistency sync (migration `013_cutover3_legacy_to_converged_sync.sql`):** the `012` views source from `control_instances` / `document_requirement_links`, populated only by the backfill, a point-in-time snapshot. Since WRITES stay legacy until cutover 4, the first legacy write after the read flip would otherwise leave the converged read stale. `013` installs TEN temporary legacy->converged sync triggers (control_states + iso42001_control_states INSERT/UPDATE, document_controls + iso42001_document_controls INSERT/DELETE/UPDATE) that mirror every legacy write into the converged tables, so the read cutover is self-consistent on its own. Direction is the OPPOSITE of the cutover-2 evidence sync (which was converged->legacy because evidence writes had already flipped); cutover 4 adds the converged->legacy direction, and with `recursive_triggers = 0` the two cannot loop. Status/applicability normalize through the same map the views use (ELSE passes raw so it surfaces as `!!UNMAPPED` / trips the CHECK, fail-loud); `last_updated` and doc-link `created_at` are copied VERBATIM so the converged read stays byte-equal to the legacy stamp continuously (the re-backfill finding, now kept fresh by trigger). NULL-`entity_id` gotcha: SQLite treats NULL as distinct in UNIQUE, so the triggers cannot use ON CONFLICT; they UPDATE the whole-org row then INSERT only WHERE NOT EXISTS. Triggers no-op when the `requirements` mapping is absent (fresh boots / the suite, where the catalog backfill has not run), so they are inert there and fire only on the live data shape. These ten triggers are TEMPORARY: dropped at the control-state demolition with the legacy tables.
...
- **Sync behavioral test** (`migrations/fixtures/cutover3_sync_check.js`, self-contained throwaway DB): on a read-flipped workspace, legacy INSERT + UPDATE + every-status round-trip + doc-link add/delete + 42001 are reflected by the converged view IMMEDIATELY, `last_updated` verbatim, exactly one converged row (no NULL-entity_id dup): **24/24 PASS**.
- Suite green throughout (smoke 45/0, node:test 25/0).

**Findings (recorded as doctrine):** (1) `'Work In Progress'` was app-producible but missing from the Phase 3 STATUS map -> closed in backfill + views; writable-vocabulary audit confirms all 6 statuses + 3 applicabilities round-trip. (2) Re-backfill-at-switch is load-bearing (the 42001 SoA `last_updated` failure). (3) Truncation-without-ordering: `computeReadiness` flag item-lists differed on ws31/ws32 only (no `ORDER BY`); fixed; bug-class hunt run across all unconverted reads (no second instance). (4) Legacy-vs-fresh schema drift: the `iso42001_control_states` legacy table carries `review_status` + `review_*` on the live DB but `roadmap_phase` + `assessment_answers` (and NO `review_status`) on a fresh `db.js` boot. A sync trigger that referenced `NEW.review_status` would throw at write time on fresh boots and break 42001 writes in the suite. DOCTRINE for triggers/queries that must run on BOTH the live instance and fresh boots: reference only the column INTERSECTION of the two schemas; verify against a fresh-boot schema dump, not just the live DB. The `013` 42001 triggers sync the safe intersection only.
...
**Demolition split (later):** `control_states` / `iso42001_control_states` / `entity_control_states` (dead) / `document_controls` / `iso42001_document_controls` demolish after cutover 4 (writes) + the standing demolition gate; `control_state_history` / `iso42001_control_state_history` demolish only after the Phase 4 engine cutover lifts the manifest's history items. The `013` sync triggers drop together with the legacy state/doc tables.

**REVIEW-CONVERGENCE MINI-STEP (hard ordering constraint, Vijay):** review stays a fully legacy workflow through cutover 4 (reads AND writes; decision 2). Before the control-state legacy tables can drop, a dedicated review-convergence mini-step must: extend `control_instances` with `reviewed_by` / `reviewed_at` / `review_reason` / `review_requested_by` (+ `review_requested_at`) via the runner; converge the review reads AND writes (review-queue + review-detail, 27001 + 42001) onto the converged schema; prove parity. **The legacy `control_states` / `iso42001_control_states` tables CANNOT be demolished while review depends on their `review_*` columns.** Scheduled immediately after cutover 4, ahead of the control-state demolition. (This also lets the `013`/`014` sync resume mirroring `review_status` for 42001, currently excluded per finding 4.)
