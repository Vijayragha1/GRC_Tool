...
//                      explicitly cannot assign capability conclusions
//
...
function isSeniorReviewer(user) {
  return !!user && user.user_type === 'firm' && ['manager', 'senior_consultant'].includes(user.firm_role);
}

function isFirmManager(user) {
  return !!user && user.user_type === 'firm' && ['manager', 'owner', 'firm_owner'].includes(user.firm_role);
}
...
function canViewEngagement(db, user, engagement) {
  ...
  // requireWorkspace has already proven the client account belongs to this workspace.
  if (user.user_type === 'client') return true;
  // Workspace middleware has already enforced firm/workspace tenancy.
  if (isFirmOperator(user)) return true;
...
function canReview(db, user, engagement) {
  if (!engagement) return false;
  if (isSeniorReviewer(user)) return true;
...
function canApprove(db, user, engagement) {
  ...
  if (!engagement) return false;
  if (isFirmManager(user)) return true;
  return hasRole(db, user.id, engagement.id, 'ENGAGEMENT_LEAD');
}

function publicationReadiness(db, engagement) {
  const defects = [];
  if (!engagement) return ['Engagement is missing.'];
  const profile = db.prepare(`SELECT status FROM csf_profile_contexts WHERE engagement_id=?`).get(engagement.id);
  if (!profile || profile.status !== 'approved') defects.push('Organizational Profile context has not been approved.');
  const unfinished = db.prepare(`SELECT COUNT(*) AS c FROM csf_subcategory_assessments
    WHERE engagement_id=? AND excluded_from_scope=0 AND status!='Approved'`).get(engagement.id).c;
  if (unfinished) defects.push(`${unfinished} in-scope Subcategories are not approved.`);
  const tiers = db.prepare(`SELECT COUNT(*) AS total,
    SUM(CASE WHEN status='approved' THEN 1 ELSE 0 END) AS approved
    FROM csf_tier_assessments WHERE engagement_id=?`).get(engagement.id);
  if (tiers.total !== 7 || tiers.approved !== 7) defects.push('Overall and all six Function Tier conclusions must be approved.');
  const unresolved = db.prepare(`SELECT COUNT(*) AS c FROM csf_reviewer_comments
    WHERE engagement_id=? AND resolved_at IS NULL AND deleted_at IS NULL`).get(engagement.id).c;
  if (unresolved) defects.push(`${unresolved} reviewer comments remain unresolved.`);
  return defects;
}

function canPublish(db, user, engagement) {
  if (!engagement || !['Approved','Published'].includes(engagement.status)) return false;
  if (!(isFirmManager(user) || hasRole(db, user.id, engagement.id, 'ENGAGEMENT_LEAD'))) return false;
  return publicationReadiness(db, engagement).length === 0;
}
...
const SUBCATEGORY_STATES = [
  ...
  'Client Validated',   // 5 - factual validation by the client
  'Approved',           // 6 - Lead approved; ready for engagement-level publish
];
...
function canTransitionTo(db, user, engagement, assessment, toState) {
  ...
  switch (toState) {
    ...
    case 'Evidence Collected':
      return canCollectEvidence(db, user, engagement) && db.prepare(`SELECT COUNT(*) AS c FROM csf_evidence_items
        WHERE assessment_id=? AND deleted_at IS NULL`).get(assessment.id).c > 0;
    case 'Draft Complete':
      // Requires scoring permission and the minimum decision-quality record.
      return canScoreSubcategory(db, user, engagement)
        && assessment.current_score != null
        && String(assessment.narrative || '').trim().length >= 120
        && !!assessment.effectiveness_conclusion
        && !!assessment.evidence_confidence;
    case 'Reviewed':
      return canReview(db, user, engagement)
        && assessment.scored_by !== user.id
        && assessment.narrative_drafted_by !== user.id
        && !assessment.is_bulk_set;
    case 'Client Validated':
      return (user.user_type === 'client' || canApprove(db, user, engagement)) && assessment.reviewed_by != null;
    case 'Approved':
      return canApprove(db, user, engagement)
        && assessment.client_validation_status === 'validated'
        && assessment.scored_by !== user.id
        && db.prepare(`SELECT COUNT(*) AS c FROM csf_reviewer_comments
          WHERE assessment_id=? AND resolved_at IS NULL AND deleted_at IS NULL`).get(assessment.id).c === 0;
    default:
      return false;
  }
}
...
function thinnessWarnings(assessment, evidenceCount) {
  ...
  if (narrative.trim().length < 120) {
    warnings.push('Narrative is incomplete - an approval-ready conclusion requires at least 120 characters.');
  }
  ...
  if (!assessment || !assessment.effectiveness_conclusion) warnings.push('Record an effectiveness conclusion.');
  if (!assessment || !assessment.evidence_confidence) warnings.push('Select an evidence-confidence level.');
  return warnings;
}
...
function canTransitionEngagement(db, user, engagement, toState) {
  ...
  const authorised = isFirmManager(user) || hasRole(db, user.id, engagement.id, 'ENGAGEMENT_LEAD');
  if (!authorised) return false;
  if (toState === 'Approved') return publicationReadiness(db, engagement).length === 0;
  return true;
}
