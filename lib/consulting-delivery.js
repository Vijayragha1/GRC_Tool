...
/**
 * Project a formally concluded ISO 27001 assessment pass into the governed
 * consulting-delivery record. The assessment pass remains the source record;
 * workpapers are immutable, reportable projections of its independently
 * signed-off conclusions and evidence links.
 */
function materializeAssessmentPass(db, ws, pass, reviewerId) {
  if (!pass || pass.status !== 'in_progress') throw new Error('Only an active assessment pass can be materialized.');
  if (Number(pass.started_by) === Number(reviewerId)) throw new Error('Assessment workpapers require an independent reviewer.');

  const engagement = ensureEngagement(db, ws, pass.started_by || reviewerId);
  const existingCount = db.prepare('SELECT COUNT(*) c FROM consultant_workpapers WHERE engagement_id=?').get(engagement.id).c;
  if (isoLifecycle.isGapOnly(ws.engagement_outcome) && !existingCount && engagement.engagement_type === 'implementation') {
    db.prepare(`UPDATE consulting_engagements SET engagement_type='gap_assessment',
      name=?,quality_reviewer_id=?,updated_at=datetime('now') WHERE id=? AND workspace_id=?`)
      .run(`${ws.client_name} ISO 27001 gap assessment`, reviewerId, engagement.id, ws.id);
  } else if (!engagement.quality_reviewer_id) {
    db.prepare(`UPDATE consulting_engagements SET quality_reviewer_id=?,updated_at=datetime('now')
      WHERE id=? AND workspace_id=?`).run(reviewerId, engagement.id, ws.id);
  }
  db.prepare(`INSERT OR IGNORE INTO consulting_engagement_team
    (engagement_id,user_id,role,assigned_by) VALUES (?,?,'consultant',?)`)
    .run(engagement.id, pass.started_by, reviewerId);
  db.prepare(`INSERT OR IGNORE INTO consulting_engagement_team
    (engagement_id,user_id,role,assigned_by) VALUES (?,?,'quality_reviewer',?)`)
    .run(engagement.id, reviewerId, reviewerId);
  ...
  const rows = db.prepare(`SELECT h.*,r.id requirement_id,r.ref requirement_ref,r.title requirement_title
    ...
    JOIN assessment_passes hp ON hp.id=h.pass_id
    JOIN requirements r ON r.ref=h.iso_item_id
    JOIN frameworks f ON f.id=r.framework_id AND f.code='iso27001'
    WHERE h.workspace_id=? AND hp.pass_number<=? AND h.id=(
      SELECT h2.id FROM control_state_history h2
      JOIN assessment_passes p2 ON p2.id=h2.pass_id
      WHERE h2.workspace_id=h.workspace_id AND h2.iso_item_id=h.iso_item_id AND p2.pass_number<=?
      ORDER BY p2.pass_number DESC,h2.snapshot_at DESC,h2.id DESC LIMIT 1
    ) ORDER BY r.sort_order,r.id`).all(ws.id, pass.pass_number, pass.pass_number);
  if (!rows.length) throw new Error('No assessment conclusions were found for this pass.');

  const evidenceFor = db.prepare(`SELECT DISTINCT e.id,e.filename,e.sha256,e.uploaded_at
    FROM evidence e
    LEFT JOIN evidence_requirement_links erl ON erl.evidence_id=e.id
    LEFT JOIN requirements er ON er.id=erl.requirement_id
    LEFT JOIN frameworks ef ON ef.id=er.framework_id
    WHERE e.workspace_id=? AND e.superseded_at IS NULL
      AND (e.iso_item_id=? OR (ef.code='iso27001' AND er.ref=?))
    ORDER BY e.id`);
  const nextRef = () => {
    const count = db.prepare('SELECT COUNT(*) c FROM consultant_workpapers WHERE engagement_id=?').get(engagement.id).c + 1;
    return `${engagement.engagement_code}-WP-${String(count).padStart(4,'0')}`;
  };
  let created = 0;
  let refreshed = 0;
  let frozen = 0;
  ...
  for (const row of rows) {
    ...
    const evidence = evidenceFor.all(ws.id, row.iso_item_id, row.iso_item_id);
    const basis = String(row.notes || row.inclusion_justification || row.exclusion_justification || '').trim();
    const rationale = basis || `The independently reviewed pass concluded ${row.status} for ${row.iso_item_id}.`;
    const clientSummary = `${row.requirement_title}: ${row.status}. ${rationale}`.slice(0, 20000);
    let workpaper = db.prepare(`SELECT * FROM consultant_workpapers
      WHERE engagement_id=? AND requirement_id=?`).get(engagement.id, row.requirement_id);
    if (workpaper && workpaper.status === 'frozen') continue;

    if (!workpaper) {
      const id = Number(db.prepare(`INSERT INTO consultant_workpapers
        (workspace_id,engagement_id,requirement_id,workpaper_ref,title,objective,procedure_performed,
         management_claim,design_conclusion,implementation_conclusion,operating_effectiveness,evidence_sufficiency,
         conclusion_rationale,client_visible_summary,client_visible,requires_client_validation,owner_id,reviewer_id,
         status,prepared_by,prepared_at,approved_by,approved_at,frozen_at,created_by)
        VALUES (?,?,?,?,?,?,?,?,?,?,'not_tested',?,?,?,1,0,?,?,'frozen',?,datetime('now'),?,datetime('now'),datetime('now'),?)`)
        .run(ws.id, engagement.id, row.requirement_id, nextRef(),
          `${row.requirement_ref} assessment workpaper`,
          `Determine and document the implementation position for ${row.requirement_ref} within the agreed ISO 27001 assessment scope.`,
          `Reviewed the saved assessment responses, consultant observations and linked evidence retained for assessment pass ${pass.pass_number}.`,
          conclusion.management_claim, conclusion.design_conclusion, conclusion.implementation_conclusion,
          conclusion.evidence_sufficiency, rationale, clientSummary,
          pass.started_by, reviewerId, pass.started_by, reviewerId, pass.started_by).lastInsertRowid);
      workpaper = db.prepare('SELECT * FROM consultant_workpapers WHERE id=?').get(id);
      created += 1;
    } else {
      db.prepare(`UPDATE consultant_workpapers SET title=?,objective=?,procedure_performed=?,management_claim=?,
        design_conclusion=?,implementation_conclusion=?,operating_effectiveness='not_tested',evidence_sufficiency=?,
        conclusion_rationale=?,client_visible_summary=?,client_visible=1,requires_client_validation=0,
        owner_id=?,reviewer_id=?,status='frozen',prepared_by=?,prepared_at=datetime('now'),approved_by=?,
        approved_at=datetime('now'),frozen_at=datetime('now'),updated_at=datetime('now'),row_version=row_version+1
        WHERE id=? AND workspace_id=?`).run(`${row.requirement_ref} assessment workpaper`,
          `Determine and document the implementation position for ${row.requirement_ref} within the agreed ISO 27001 assessment scope.`,
          `Reviewed the saved assessment responses, consultant observations and linked evidence retained for assessment pass ${pass.pass_number}.`,
          conclusion.management_claim, conclusion.design_conclusion, conclusion.implementation_conclusion,
          conclusion.evidence_sufficiency, rationale, clientSummary, pass.started_by, reviewerId,
          pass.started_by, reviewerId, workpaper.id, ws.id);
      workpaper = db.prepare('SELECT * FROM consultant_workpapers WHERE id=?').get(workpaper.id);
      refreshed += 1;
    }

    for (const item of evidence) {
      db.prepare(`INSERT INTO consultant_workpaper_evidence
        (workpaper_id,evidence_id,purpose,relevance,reviewer_note,linked_by)
        VALUES (?,?,'Supports the signed-off assessment conclusion','relevant',?,?)
        ON CONFLICT(workpaper_id,evidence_id) DO UPDATE SET relevance='relevant',reviewer_note=excluded.reviewer_note`)
        .run(workpaper.id, item.id, `Reviewed during assessment pass ${pass.pass_number}.`, reviewerId);
    }
    const priorStatus = workpaper.status === 'frozen' ? 'approved' : workpaper.status;
    db.prepare(`INSERT INTO consultant_workpaper_reviews
      (workpaper_id,review_type,decision,note,from_status,to_status,actor_id)
      VALUES (?,'consultant_submission','submitted',?,'draft','manager_review',?)`)
      .run(workpaper.id, `Submitted through assessment pass ${pass.pass_number}.`, pass.started_by);
    db.prepare(`INSERT INTO consultant_workpaper_reviews
      (workpaper_id,review_type,decision,note,from_status,to_status,actor_id)
      VALUES (?,'manager_review','approved',?,'manager_review','approved',?)`)
      .run(workpaper.id, `Independent sign-off of assessment pass ${pass.pass_number}.`, reviewerId);
    db.prepare(`INSERT INTO consultant_workpaper_reviews
      (workpaper_id,review_type,decision,note,from_status,to_status,actor_id)
      VALUES (?,'freeze','frozen',?,'approved','frozen',?)`)
      .run(workpaper.id, `Frozen from signed-off assessment pass ${pass.pass_number}.`, reviewerId);
    const frozenRow = workpaperDetail(db, ws, workpaper.id);
    const snapshot = {
      source: { type: 'iso27001_assessment_pass', id: pass.id, pass_number: pass.pass_number },
      workpaper: frozenRow,
      evidence: evidenceSnapshot(db, frozenRow),
      reviews: db.prepare(`SELECT review_type,decision,note,from_status,to_status,actor_id,created_at
        FROM consultant_workpaper_reviews WHERE workpaper_id=? ORDER BY id`).all(workpaper.id),
      captured_at: new Date().toISOString()
    };
    const json = JSON.stringify(snapshot);
    const version = db.prepare('SELECT COUNT(*) c FROM consultant_workpaper_snapshots WHERE workpaper_id=?').get(workpaper.id).c + 1;
    db.prepare(`INSERT INTO consultant_workpaper_snapshots
      (workpaper_id,version_number,snapshot_json,snapshot_hash,frozen_by) VALUES (?,?,?,?,?)`)
      .run(workpaper.id, version, json, sha(json), reviewerId);
    event(db, ws.id, engagement.id, reviewerId, 'workpaper', workpaper.id, 'assessment_pass_frozen', {
      pass_id: pass.id, pass_number: pass.pass_number, evidence_count: evidence.length, prior_status: priorStatus
    });
    frozen += 1;
  }
  event(db, ws.id, engagement.id, reviewerId, 'assessment_pass', pass.id, 'materialized', {
    pass_number: pass.pass_number, created, refreshed, frozen
  });
  return { engagementId: engagement.id, created, refreshed, frozen };
}
...
module.exports = {
  ...
  requirementCatalog,event,portfolio,sha,clean,validDate,materializeAssessmentPass
};
