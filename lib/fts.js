...
function search(workspaceId, query, limit = 50) {
  ...
  // Auto-rebuild on staleness, not just emptiness. The old behaviour rebuilt
  // only when the index was empty, so any entity added after the first search
  // was invisible until someone clicked "Rebuild index" manually. Now we
  // compare the index row count to the sum of source-table counts for this
  // workspace; if they don't match, rebuild. Cheap on typical workspaces.
  try {
    ...
    const src = db.prepare(`SELECT
      ...
      + (SELECT COUNT(*) FROM ${ctlReads.tables(db, workspaceId).cs} WHERE workspace_id=?)
    ...
    switch (entityType) {
      ...
      case 'control':
        ...
        row = db.prepare(`SELECT i.title, cs.notes FROM iso_items i
          LEFT JOIN ${ctlReads.tables(db, workspaceId).cs} cs ON cs.iso_item_id=i.id AND cs.workspace_id=?
...
function rebuildAll(workspaceId) {
  ...
  for (const c of db.prepare(`SELECT cs.iso_item_id, i.title, cs.notes FROM ${ctlReads.tables(db, workspaceId).cs} cs INNER JOIN iso_items i ON i.id=cs.iso_item_id WHERE cs.workspace_id=?`).all(workspaceId)) {
...
module.exports = { indexEntity, removeEntity, refresh, search, rebuildAll };
