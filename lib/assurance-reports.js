...
function supplierData(db, wsId, supplierId, manifest) {
  ...
  const ddqEvidence = ddqAssessments.length ? db.prepare(`SELECT * FROM supplier_ddq_evidence WHERE assessment_id IN (${ddqAssessments.map(() => '?').join(',')}) ORDER BY uploaded_at,id`).all(...ddqAssessments.map(row => row.id)) : [];
  ...
  const currentDecision = decisions.find(row => !row.superseded_at);
  ...
  supplier.residual_risk_score = currentDecision ? currentDecision.residual_risk_score : null;
  supplier.residual_risk_band = currentDecision ? currentDecision.residual_risk_band : null;
  ...
  return { supplier, decisions, documents, subprocessors, clauses, reviews, monitoring, findings,
    inherentAssessments, inherentResponses, ddqAssessments, ddqResponses, ddqEvidence, contractReviews, contractItems };
}
...
function qualityChecks(key, data) {
  ...
  if (key === 'supplier_due_diligence') {
    ...
    if (!s.inherentAssessments.some(row => row.status === 'approved')) add('critical', 'inherent_assessment_missing', 'No approved workbook-governed inherent-risk assessment exists.', `/workspaces/${c.workspace.id}/vendors/${s.supplier.id}/inherent-risk`);
    ...
    if (!s.ddqAssessments.some(row => row.status === 'complete')) add('critical', 'questionnaire_unreviewed', 'No scoped supplier due-diligence assessment has completed internal review.', `/workspaces/${c.workspace.id}/vendors/${s.supplier.id}/due-diligence`);
    if (!s.contractReviews.some(row => row.status === 'complete')) add('warning', 'contract_review_incomplete', 'The 47-clause contract and SLA review is not complete.', `/workspaces/${c.workspace.id}/vendors/${s.supplier.id}/contract-review`);
...
function reportBody(run, snapshot, quality, manifest) {
  ...
  if (selected.has('executive_summary')) {
    const summary = snapshot.metadata.executive_summary || (snapshot.report_key === 'supplier_due_diligence'
      ? `${s.supplier.name} has ${s.supplier.residual_risk_band ? `a ${s.supplier.residual_risk_band}` : 'no approved'} residual-risk position. This report preserves the evidence and decision context available at the cutoff date.`
  ...
  if (s && selected.has('questionnaire')) { const latest=s.ddqAssessments.find(r=>r.status==='complete')||s.ddqAssessments[0]; const rows=latest?s.ddqResponses.filter(r=>r.assessment_id===latest.id):[]; const contract=s.contractReviews.find(r=>r.status==='complete')||s.contractReviews[0]; const contractRows=contract?s.contractItems.filter(r=>r.review_id===contract.id):[]; out.push(section('Due-diligence assessment','Tiered supplier response, evidence and review',`${latest?`<div class="metrics">${metric('Scope',supplierRisk.tierLabel(latest.tier))}${metric('Status',latest.status)}${metric('Questions',rows.length)}${metric('Open review items',rows.filter(r=>r.status!=='Response Complete').length)}</div>`:''}${table(['Domain','Question','Vendor response','Evidence','Reviewer conclusion','Status'],rows.map(r=>`<tr>${td(r.domain)}${td(`${r.question_id} · ${r.question}`)}${td(r.response)}${td(r.evidence_reference)}${td(r.reviewer_conclusion)}${td(r.status)}</tr>`),'No governed DDQ responses recorded.')}${table(['Contract clause','Status','Reference','Finding'],contractRows.map(r=>`<tr>${td(r.clause_id)}${td(r.status)}${td(r.contract_reference)}${td(r.finding_id)}</tr>`),'No contract review recorded.')}`)); }
  ...
  if (s && selected.has('decision')) out.push(section('Approval decision', 'Accountable risk acceptance', table(['Decision','Date','Decider','Residual-risk band','Valid until','Rationale'], s.decisions.map(r => `<tr>${td(r.decision)}${td(date(r.decided_at))}${td(r.decider_name || r.actor_name)}${td(r.residual_risk_band)}${td(date(r.valid_until))}${td(r.rationale)}</tr>`), 'No approval decision recorded.')));
...
function docxReportBody(run, snapshot, quality, manifest) {
  ...
  const out = [];
  const summary = snapshot.metadata.executive_summary || (snapshot.report_key === 'supplier_due_diligence'
    ? `${s.supplier.name} has ${s.inherentAssessments.find(row=>row.status==='approved') ? `${s.inherentAssessments.find(row=>row.status==='approved').assigned_tier.replace('_',' ')} inherent risk at ${s.inherentAssessments.find(row=>row.status==='approved').weighted_score}/100` : 'no approved inherent-risk position'}${s.decisions.find(row=>!row.superseded_at) && s.decisions.find(row=>!row.superseded_at).residual_risk_band ? ` and a ${s.decisions.find(row=>!row.superseded_at).residual_risk_band} residual-risk conclusion` : ''}. This report preserves the evidence and decision context available at the cutoff date.`
    : `${c.workspace.client_name} has ${c.stats.readiness_pct}% calculated control readiness at the reporting cutoff, with ${c.stats.risks_critical} critical residual risks and ${c.stats.ncs_open} open nonconformities.`);
  ...
  if (s && selected.has('questionnaire')) { const latest=s.ddqAssessments.find(r=>r.status==='complete')||s.ddqAssessments[0]; const rows=latest?s.ddqResponses.filter(r=>r.assessment_id===latest.id):[]; const contract=s.contractReviews.find(r=>r.status==='complete')||s.contractReviews[0]; const contractRows=contract?s.contractItems.filter(r=>r.review_id===contract.id):[]; out.push(docSection('Due-diligence assessment','Tiered supplier response, evidence and review',`${latest?docMetrics([['Scope',supplierRisk.tierLabel(latest.tier)],['Status',latest.status],['Questions',rows.length],['Open review items',rows.filter(r=>r.status!=='Response Complete').length]]):''}${table(['Domain','Question','Vendor response','Evidence','Reviewer conclusion','Status'],rows.map(r=>`<tr>${td(r.domain)}${td(`${r.question_id} · ${r.question}`)}${td(r.response)}${td(r.evidence_reference)}${td(r.reviewer_conclusion)}${td(r.status)}</tr>`),'No governed DDQ responses recorded.')}${table(['Contract clause','Status','Reference','Finding'],contractRows.map(r=>`<tr>${td(r.clause_id)}${td(r.status)}${td(r.contract_reference)}${td(r.finding_id)}</tr>`),'No contract review recorded.')}`,true)); }
  if (s && selected.has('evidence')) out.push(docSection('Supplier evidence','Document integrity and currency',`${table(['Document','Type','Effective','Expires','SHA-256'],s.documents.map(r=>`<tr>${td(r.name)}${td(r.doc_type)}${td(date(r.effective_date))}${td(date(r.expiry_date))}${td(r.sha256?r.sha256.slice(0,16)+'…':'')}</tr>`),'No supplier assurance documents attached.')}${table(['DDQ question','Evidence file','Source','Uploaded','SHA-256'],s.ddqEvidence.map(r=>`<tr>${td(r.question_id)}${td(r.filename)}${td(r.source)}${td(date(r.uploaded_at))}${td(r.sha256?r.sha256.slice(0,16)+'…':'')}</tr>`),'No DDQ evidence files attached.')}`,true));
  ...
  if (s && selected.has('decision')) out.push(docSection('Approval decision','Accountable risk acceptance',table(['Decision','Date','Decider','Residual-risk band','Valid until','Rationale'],s.decisions.map(r=>`<tr>${td(r.decision)}${td(date(r.decided_at))}${td(r.decider_name||r.actor_name)}${td(r.residual_risk_band)}${td(date(r.valid_until))}${td(`${r.rationale||''}${r.residual_risk_rationale?` · ${r.residual_risk_rationale}`:''}`)}</tr>`),'No approval decision recorded.'),true));
