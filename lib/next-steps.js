...
function computeNextStep(ws) {
  ...
  // cert-deadline is auto-seeded at client creation, so counting it made a
  // brand-new client look as though setup had begun.
  const intakeCount = cnt(`SELECT COUNT(*) c FROM engagement_intake WHERE workspace_id=? AND question_id<>'cert-deadline' AND answer IS NOT NULL AND length(trim(answer)) > 0`);
  if (intakeCount === 0) {
    return {
  ...
  if (intakeCount < 8) {
    return {
      kind: 'intake-partial', title: `Finish the client setup (${intakeCount} answered)`,
